...
CKPT_ARGS=(
   ...
   --save /root/slime_siqi/output/Qwen3-8B_opsd_hidden_think_openthoughts/
   --save-interval 2000
)
...
GRPO_ARGS=(
   ...
   --opsd-teacher-info-mode hidden_think

   --opsd-jsd-coef 1.0

   # Top-k forward KL，保留 top 100 个 teacher tokens
   # --opsd-loss-type topk_forward_kl
   # --opsd-topk 50
...
WANDB_ARGS=(
   ...
   --wandb-group qwen3-8B-opsd_hidden_think-forward_kl+ref_kl-openthoughts-kl05-nothinking
...
unset RAY_ADDRESS
ray stop --force || true
export CUDA_VISIBLE_DEVICES=1,3,4,6,7
ray start --head --node-ip-address ${MASTER_ADDR} --num-gpus 5 --disable-usage-stats --dashboard-host=0.0.0.0 --dashboard-port=8265
...
ray job submit --address="http://127.0.0.1:8265" \
   --runtime-env-json='{
     "env_vars": {
        ...
        "CUDA_VISIBLE_DEVICES": "1,3,4,6,7"
     }
   }' \
   -- python3 train.py \
   --actor-num-nodes 1 \
