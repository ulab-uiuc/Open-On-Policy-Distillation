...
CKPT_ARGS=(
   ...
   --save /root/slime_siqi/output/Qwen2.5-1.5B_opsd_answeronly_dapo/
   --save-interval 2000
)
...
GRPO_ARGS=(
   ...
   --opsd-loss-type decoded_kl
   # --opsd-topk-source student
   # choices=["jsd", "reverse_kl", "forward_kl", "wiener_kl", 
         #   +"topk_forward_kl", "topk_tail_kl", "decoded_kl", "kl_biased_ppo"],
   # NOTE: decoded_kl does not use --opsd-topk / --opsd-topk-source /
   # --opsd-kl-ppo-eta / --opsd-kl-ppo-tau.

   # Unused by decoded_kl (only used when --opsd-loss-type=kl_biased_ppo).
...
WANDB_ARGS=(
   ...
   --wandb-group qwen2.5-1.5B-opsd_answeronly-dapo
...
unset RAY_ADDRESS
ray stop --force || true
export CUDA_VISIBLE_DEVICES=2,3,5,9
